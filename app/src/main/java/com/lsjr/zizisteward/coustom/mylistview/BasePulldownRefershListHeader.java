...
public abstract class BasePulldownRefershListHeader extends LinearLayout {
	...
	public BasePulldownRefershListHeader(Context context) {
		...
		contentViewHeight = ViewUtil.getMeasuredHeight(contentView); // 测量内容的高度
		contentView.setLayoutParams(new LinearLayout.LayoutParams(
				android.view.ViewGroup.LayoutParams.MATCH_PARENT, contentViewMinHeight)); // 设置内容视图的高度为最小高度
	...
	public void updateHeight(int newHeight) {
		...
		if (state == State.NORMAL) { // 如果当前状态是正常
			if (newHeight >= contentViewHeight
					...
					.isAllowReadyRefresh())) { // 如果新的高度大于原始高度，说明是要从正常状态进入准备刷新状态了
	...
	public int getRollbackFinalHeight() {
		return state == State.REFRESHING ? contentViewHeight
				: contentViewMinHeight; // 如果正在刷新就回滚到原始高度，否则回滚到最小高度
	}
